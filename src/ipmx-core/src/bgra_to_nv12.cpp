    ...
    for (uint32_t column = 0; column < source.width; column += 2U) {
      ...
      for (uint32_t dy = 0; dy < 2U; ++dy) {
        ...
        for (uint32_t dx = 0; dx < 2U; ++dx) {
          ...
          result.y_plane()[static_cast<size_t>(row + dy) * result.y_stride + column + dx] =
              clamp_byte(y709(input[(column + dx) * 4U + 2U],
                             input[(column + dx) * 4U + 1U], input[(column + dx) * 4U]));
        }
      }
      r = (r + 2) / 4;
      g = (g + 2) / 4;
